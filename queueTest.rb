	...
	if results.count == 0  
		...
		messages_arr.each do |raw|
			...
			@db.query("CREATE TABLE IF NOT EXISTS `BESUCH` (
				...
				KEY `LOCATION_START` (`LOC_NAME`, `START_TIME`),
				KEY `Device_Start` (`DEVICE_ID`,`START_TIME`),
				KEY `LOCATION_END` (`LOC_NAME`, `END_TIME`),
				KEY `Device_End` (`DEVICE_ID`,`END_TIME`),
				KEY `Loc_Device_Start` (`LOC_NAME`,`DEVICE_ID`,`START_TIME`)
				) ENGINE=innodb;")
				...
				#START_TIME / END TIME
				start_time = iso8601_to_mysql_datetime("#{visit_time}")
				...
				# If it doesn't exist, INSERT, otherwise, UPDATE
				time = Time.now
				end_time = iso8601_to_mysql_datetime("#{time}")
				...
				if @db.query("select device_id from besuch where device_id like '#{device_id}'") != FALSE #if mac_address is not found is black list then => 
					##Make besuch 'besuch_blacklist' once we get a blacklist together
					@db.query("
