...
messages_arr.each do |raw|
  ...
  count = 0
  end_time = start_time
  min_signal = clean_device.max_signal
  ...
  @db.query("
    ...
    VALUES
    ('#{clean_device.device_id}', '#{clean_device.mac_prefix}', '#{clean_device.location_id}', '#{visit_date}', '#{start_time}', '#{end_time}', '#{count}', '#{min_signal}', '#{clean.max_signal}')
