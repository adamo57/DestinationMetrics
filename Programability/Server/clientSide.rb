...
#recieved_message = @queue.receive_messages(wait_time_seconds: 15, visibility_timeout: 10)
...
messages_arr.each do |raw|
  ...
  insert_device = Device.new(data["Device_ID"], data["MAC_Prefix"],data["Location_Name"], data["visit_time"], data["visit_db"])
  ...
  count = 0
  end_time = start_time
  min_signal = insert_device.max_signal
  ...
  @db.query("
    ...
    VALUES
    ('#{insert_device.device_id}', '#{insert_device.mac_prefix}', '#{insert_device.location_id}', '#{visit_date}', '#{start_time}', '#{end_time}', '#{count}', '#{min_signal}', '#{insert_device.max_signal}')
